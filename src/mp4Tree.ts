...
export class MP4Tree {
  ...
  async setTag(outPath: string, tags: {
    ...
    if (tags.track) {
      ...
      let [track, total] = tags.track.split('/').map(i => parseInt(i));
      if (typeof track === "number" && Number.isInteger(track) && track >= 0 && track < 65536) {
        trackString += "\0\0" + track; //TODO
      ...
      if (Buffer.isBuffer(tags.cover)) {
        ...
        try {
          ...
          coverBuffer = await stat(coverPath).then(
            stats => {
              // TODO: STATS ERROR HANDLE
              outBuffer = Buffer.alloc(stats.size);
              return open(coverPath);
            }).then(fd => fd.read({ buffer: outBuffer })
          ).then(
            res => res.buffer
          );
        ...
        console.log("[MP4Tree ERROR] Failed to read cover " + tags.cover + ": Require path string or Buffer as input.");
      }
    }
